...
Supervisor's own paraphrase -- this project's own invariant, measured in hl8 as a
...
a rendered `ToolMessage` and a state write; `research` returns its
...
`configurable={"thread_id": uuid4()}` is what actually keeps "no sub-agent
carries a checkpointer" true at runtime.
...
class SupervisorState(AgentState[None]):
    """`AgentState` plus what the revision-cap and save-guard middlewares
    read and write.
...
def _original_request(messages: Sequence[Any]) -> str:
    ...
    This project's own code-not-prompt invariant: every wrapper forwards this in
...
def _sanitized_sub_agent_config(config: RunnableConfig) -> RunnableConfig:
    ...
    -- only a config that itself supplies a
    ...
    graph (which has none of its own).
    """
...
def create_supervisor(
    ...
    *,
    ...
    role_models : Mapping of str to BaseChatModel
        ...
        can still name the `models`/`tools` modules (this function's own assembly reads
