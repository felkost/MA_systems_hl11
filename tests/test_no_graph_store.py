...
This project bans a graph store outright -- it is a second source of truth
this assignment does not measure.

Checked with `ast`, not a text search: a naive substring search on
`"graph_search"`/`"neo4j"` also matches every *legitimate* mention --
this module's own docstring saying "minus `graph_search`", and
