"""Planner component tests -- GEval `Plan Quality`.
...
were built at import time (measured directly against the installed package).
...
Live inputs are golden `happy_path` cases,
...
def _run_case(
    ...
    *,
    ...
    # ; without it DeepEval's persisted file carries no way back to
...
def test_planner_recognizes_incoherent_nonsense_as_out_of_scope(
    ...
    """`p2` -- deterministic, no judge: `failure-nonsense-query`'s
    ...
    threshold, at the live agent's own cost alone (~$0.005 measured).
    Cost and spans are still persisted (`record_case_cost`
