...
a precise hook contract, and a rename
...
# -- Test 14: this project's own renamed tool set, vacuous until the
# coordinator exists by design --
...
def test_stability_tools_match_the_supervisor_tool_names() -> None:
    """Pinned against this project's own delegation tool names (hl8's `_S1`:
    `plan`, `research`, `critique`, `save_report`), not against
    `supervisor.py` itself -- the same shape as
...
#
# A real live run produced two cases whose `save_report` was refused by
...
# a real run once found stuck: a save_report refused by
