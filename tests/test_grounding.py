"""`grounding.py` -- the extractor and the run-scoped
nudge.

The extractor's own false-positive gate needs a **real** saved report, not a
...
(a real live checkpoint) -- both are named explicitly, since a
...
def test_bare_title_case_token_alone_does_not_qualify() -> None:
    # A plain title-case word alone should not qualify: no digit, not
    # all-caps.
...
def test_false_positive_ceiling_on_a_real_saved_report(fixture_name: str) -> None:
    # A measured bound for the revised extractor: 4-17 candidates
