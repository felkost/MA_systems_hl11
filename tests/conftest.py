...
session-scoped for the same reason: one
...
total, and one fixture HTTP server is enough for the one
case that needs it.
"""
...
def live_settings(tmp_path_factory: pytest.TempPathFactory) -> Iterator[Settings]:
    ...
    `runs/` to avoid polluting it, with observability configured exactly
...
def fixture_base_url() -> Iterator[str]:
    ...
    . Only `adversarial-indirect-injection` uses it
...
@pytest.fixture(scope="session")
def eval_run_id() -> str:
    """One id for this whole e2e evaluation session --
    a different namespace from each case's own Supervisor `run_id`.
...
def e2e_judge_model(live_settings: Settings) -> OpenRouterModel:
    ...
    3 metrics, with a real `usage_log` -- sharing one
...
def component_judge_model(live_settings: Settings) -> OpenRouterModel:
    ...
    files' own live cases, extended past `test_e2e.py`'s own fixture.

    Component-file judge cost had never been measured before this instance
    existed -- each test body built a fresh, unlogged judge model instead.
    Separate from `e2e_judge_model` because the two never run in the same
    process (a two-invocation configuration keeps `tests/test_e2e.py` and
    the four component files in separate `deepeval test run` invocations),
    not because sharing one instance across both would be wrong.
    """
    return _shared_judge_model(live_settings)
...
def run_judged_case(
    ...
    *,
    ...
    failure into the raised error, which no component-file case
    ...
    Exception
        ...
        crash as well as an ordinary metric failure (measured live: a
...
def persist_case_spans(eval_run_id: str, *, case_id: str, spans: RunSpans) -> None:
    ...
    `runs/<eval_run_id>/spans/<case_id>.json`, before the
    `tmp_path_factory` root that produced it is cleaned up.

    Diagnosis was lost to exactly that cleanup more than once before this
    existed: the `tmp_path_factory`-rooted span dump `live_settings` redirects to does
