...
CLI command itself, in-process, right after `pytest.main()` returns
(measured directly against the installed package). A bare
`pytest tests/test_e2e.py` never writes it.
...
# (measured directly), so metric identity is the only join available --
...
class UnknownCaseError(RuntimeError):
    ...
    fails loudly rather than silently dropping it from the summary, per this
    project's own "fail loudly on an unrecognised mapping" discipline."""
...
def load_latest_run(path: Path | None = None) -> dict[str, Any]:
    ...
    path : Path, optional
        ...
        since the **second** `deepeval test run` invocation overwrites
...
def filter_e2e_cases(
    ...
    """Keep only `testCases` entries scored by this project's own three
    e2e metrics, tagged with their golden-dataset category.

    Filters on metric identity (names this module itself owns), not on the
    case name's own shape -- robust regardless of how many other eval-tier
    files (the component and tool-correctness tests) were collected
    in the same `deepeval test run` invocation.

    Membership, not equality: an e2e case carries one
    ...
    denominator** -- silently, the same failure shape a snapshot
    mechanism elsewhere exists to prevent. Guarded against an empty
...
def render_summary_markdown(
    ...
    *,
    ...
    total_cases : int
        ...
        skipped, e.g. the poisoned-knowledge-base case) states it
...
def main(eval_run_id: str | None = None, *, run: dict[str, Any] | None = None) -> Path:
    ...
    eval_run_id : str, optional
        ...
        own directory.
    run : dict, optional
        An already-parsed DeepEval run -- when supplied,
