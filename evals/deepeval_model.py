...
Deliberately has no environment or `Settings` coupling of its own. The
constructor takes `api_key` and `model_name` explicitly; the caller (a
test, or `evals/runner.py`) is what reads `Settings` or the environment.
...
optional `usage_log` accumulates
...
class OpenRouterModel(DeepEvalBaseLLM):
    ...
    def __init__(
        ...
        # 180s, raised from 120, itself raised from 60
        # before that. Each raise was made against a measured
        ...
        #
        # The ceiling on this value is the invariant this project names:
        ...
        # (`AnswerRelevancyMetric` makes three): reading `last_usage`
        # after the fact only ever sees the last of them. A caller that
        # passes a shared list here (e.g. an `e2e_judge_model` fixture)
        ...
        # `None` (the default) keeps the payload byte-for-byte what
    ...
    def _payload(self, prompt: str, schema: Type[BaseModel] | None) -> dict[str, Any]:
        ...
        if schema is not None:
            ...
            # `models.assert_structured_output_supported` will
