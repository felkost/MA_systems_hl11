...
belong to `supervisor.py`. They ship here anyway --
splitting one module in two costs more than it saves, the same
call hl10 made landing `CritiqueResult` before its Critic existed. Tested
here as isolated units against hand-built state; the Supervisor's own tests
prove them wired.
"""
...
# A rename of hl10's `_STABILITY_TOOLS`. This project's Supervisor
...
class CriticVerificationMiddleware(
    ...
    first call still gets its own `model.<role>` span (a real finding:
...
class SaveReportGuardMiddleware(
    ...
    *executed* `save_report` exists since that boundary, and the verdict is
    ...
    with nothing but the refusal string to act on, and two real cases
    simply stopped there and answered the user in prose
    ...
    def _nudge_for(
        ...
        if request.state.get("verdict") == "APPROVE":
            ...
            # silent on exactly the runs a real trace found stuck: a
...
def _set_model_call_usage_attributes(
    ...
    # and cost columns from: measured directly, a full run's 454
...
class TracingMiddleware(AgentMiddleware[AgentState[ResponseT], ContextT, ResponseT]):
    ...
    tracer directly
    (`trace.get_tracer(__name__)`), never `observability.py` itself
    -- the span this creates nests under whatever `main.py`'s per-turn
    `repl.question` span or a delegation-site `agent.<role>` span
...
def agent_middleware(
    *,
    ...
    list of AgentMiddleware
        ...
        agent factories never assemble any part of their own stack.
